...
const FormThree = () => {
    ...
    const FileUploader = ({ name }) => {
        const { getRootProps, getInputProps, isDragActive } = useDropzone({
            onDrop: (acceptedFiles) => {
                // Keep only the first file for each field
                setFiles((prevFiles) => ({
                    ...prevFiles,
                    [name]: acceptedFiles[0] ? acceptedFiles[0] : null, // Store the single file, or null
                }));
            },
    ...
    const handleSubmit = async (e) => {
      ...
      // Append files to formData *only* if they exist.  Critical!
      for (const key in files) {
          if (files.hasOwnProperty(key) && files[key]) { // Use hasOwnProperty and check for null
      ...
      try {
          const response = await axios.post(
              `${import.meta.env.VITE_API_URL}/form-three`, // Corrected URL: no extra /form
              formData,
              {
          ...
          navigate("/next-step"); // Example: Navigate to a success page
  
      } catch (error) {
          console.error("File upload error:", error.response || error);
          // Improve error handling: Check for different types of errors
          if (error.response) {
    ...
    // Document Lists (No changes needed here)
    const homeSalariedDocuments = [
        ...
        "2 years Form 16 (Part A & B) and 26 AS",
                                ...
                                <table className="w-full border-collapse border border-gray-300">
                                    ...
                                    <tbody>
                                        {getDocumentList().map((doc, index) => {
                                            const name = doc.toLowerCase().replace(/\s+/g, "_").replace(/[^a-z0-9_]/g, ""); // Sanitize field name
                                            return (
