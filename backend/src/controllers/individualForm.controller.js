//individualForms.controllers.js
...
const createFormOne = async (req, res) => {
    try {
        ...
        // Validation:  Father's name is NOT required.
        if (!full_name || !mobile_number || !email_id) {
            return res.status(400).json({ error: "Full name, mobile number, and email are required fields." });
        }
        ...
        //  Still handle potential duplicate email errors, but don't pre-check.
        if (error.code === 11000) {
            return res
                .status(400)
                .json({
                    error: "Duplicate key error.  This likely means an email or other unique field already exists.",
                });
        }
        res
            .status(500)
...
// Remainder functions are not changed
...
const uploadLoanDocuments = async (req, res) => {
    try {
        ...
        for (const [fieldName, fileArray] of Object.entries(req.files)) {
            documentData[fieldName] = fileArray[0].filename; // Save the filename
        }
